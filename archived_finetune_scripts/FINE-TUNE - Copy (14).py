...
ID = "14"
DATASET_ID = "6"
MODEL_NAME = "models/Llama-3.1-8B"  # Path to local model
DATASET_PATH = f"./datasets/mcv/100_minecraft_villager_dataset_formatted_{DATASET_ID}-llama.jsonl"  # Path to local dataset
TOKENS_PATH = f"./datasets/mcv/100_minecraft_villager_dataset_tokens_{DATASET_ID}-llama.json"  # Path to the tokens json
OUTPUT_DIR = f"./finetuned/{ID}_llama_3_1_8b_instruct"  # Path to save the fine-tuned model
EPOCHS = 24 # Increased number of epochs for better fine-tuning
BATCH_SIZE = 1  # Adjusted batch size to fit the GPU (RTX 4070 Ti Super)
LEARNING_RATE = 5e-5
LOGGING_STEPS = 2

FREEZE = False  # Freeze all layers except the first few
...
def format_conversation(conversation):
    ...
    for turn in conversation:
        ...
        formatted_text += (
            f"User: {user_text}\n"
            f"Assistant: {assistant_text}\n\n"  # Add an extra newline to separate turns
        )
    ...
    if FREEZE:
        for name, param in model.named_parameters():
            if 'model.layers.' in name:
                ...
                if layer_num >= 6 and layer_num <= 25:  # Freeze layers after the 6th layer
...
def train_model():
    ...
    training_args = TrainingArguments(
        ...
        weight_decay=0.01,
        bf16=True,
        max_grad_norm=1,  # Enable gradient clipping

        #deepspeed="./deepspeed_config.json",  # Add DeepSpeed config
    )
