...
OUTPUT_DIR = "./finetuned/6_llama_3_1_8b_instruct_abcd"  # Path to save the fine-tuned model
EPOCHS = 2
BATCH_SIZE = 4
LEARNING_RATE = 3e-5
LOGGING_STEPS = 10
...
def tokenize_function(example):
    ...
    return {k: v.squeeze(0) for k, v in encoding.items()}  # Remove extra dimensions
...
training_args = TrainingArguments(
    ...
    warmup_steps=100,  # NEW: Gradual warmup for stable start
    lr_scheduler_type="cosine",  # NEW: Cosine decay for smoother convergence
    logging_dir="./logs",
    logging_steps=LOGGING_STEPS,
    save_steps=20,
    save_total_limit=2,
    max_grad_norm=1.0, 
    gradient_accumulation_steps=2,  # NEW: Increase to simulate a larger batch
    run_name="llama_3_1_8b_instruct_abcd",
    #report_to="tensorboard",
    bf16=True,  # NEW: Enable BF16 training
)
...
# If folder exists (output_dir + checkpoint-*), resume training
dirs = os.listdir(OUTPUT_DIR)
if any("checkpoint" in d for d in dirs):
