...
OUTPUT_DIR = "./finetuned/32_results"
...
with torch.no_grad():
    ...
    new_embeddings[old_emb_size:] = new_embeddings[:old_emb_size].mean(dim=0) * 1.5

    new_embeddings2 = model.get_output_embeddings().weight.data
    old_emb_size2 = new_embeddings2.shape[0] - len(SPECIAL_TOKENS)
    new_embeddings2[old_emb_size2:] = new_embeddings2[:old_emb_size2].mean(dim=0) * 1.5
